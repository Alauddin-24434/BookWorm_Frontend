...
import { Star, BookOpen, Bookmark, CheckCircle, Clock } from "lucide-react";
...
async function getBookDetails(id: string) {
  const res = await fetch(`http://localhost:5000/api/v1/books/${id}`, { next: { revalidate: 60 } });
...
const { id } = await params; 
  ...
  if (!book) return <div className="py-20 text-center font-serif text-2xl">Book not found</div>;

  return (
    <div className="bg-[#faf9f6] min-h-screen pb-20">
      <div className="max-w-6xl mx-auto px-4 py-12">
        {/* Main Hero Card */}
        <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden mb-12 flex flex-col md:flex-row">
          ...
          <div className="md:w-2/3 p-10 md:p-16 flex flex-col justify-center">
            ...
            <h1 className="text-5xl font-serif font-bold text-slate-900 leading-tight mb-4">{book.title}</h1>
            <p className="text-2xl text-slate-500 font-serif italic mb-8">by {book.author}</p>
            ...
            <div className="flex items-center gap-8 mb-8 pb-8 border-b border-slate-100">
               <div>
                 <p className="text-3xl font-bold text-slate-900 flex items-center gap-1">
                   {book.averageRating.toFixed(1)} <Star className="w-6 h-6 fill-yellow-400 text-yellow-400" />
                 </p>
                 <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Avg Rating</p>
               </div>
               <div className="w-px h-10 bg-slate-100" />
               <div>
                 <p className="text-3xl font-bold text-slate-900">{book.totalReviews}</p>
                 <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Reviews</p>
               </div>
            </div>

            <p className="text-slate-600 text-lg leading-relaxed mb-10">{book.description}</p>

            {/* Client Component for Shelf Actions */}
            <ShelfActions bookId={book._id} />
          </div>
        </div>

     {/* Form: Left Sticky Column */}
