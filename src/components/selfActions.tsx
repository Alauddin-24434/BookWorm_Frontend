...
export default function ShelfActions({ bookId }: { bookId: string }) {
  ...
  const addToShelf = async (shelfType: "wantToRead" | "currentlyReading" | "read") => {
    ...
    try {
      ...
      const label = shelfType === "wantToRead" ? "Want to Read" : 
                    shelfType === "currentlyReading" ? "Currently Reading" : "Read";
  ...
  return (
    <div className="flex flex-wrap gap-4 pt-6">
      ...
      >
        {loadingShelf === "wantToRead" ? <Loader2 className="w-5 h-5 animate-spin" /> : <Bookmark className="w-5 h-5" />}
        Want to Read
      </button>
      ...
      >
        {loadingShelf === "currentlyReading" ? <Loader2 className="w-5 h-5 animate-spin" /> : <BookOpen className="w-5 h-5" />}
        Reading
      </button>
      ...
      >
        {loadingShelf === "read" ? <Loader2 className="w-5 h-5 animate-spin" /> : <CheckCircle className="w-5 h-5" />}
        Finished
      </button>
    </div>
  );
